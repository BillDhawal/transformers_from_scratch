...
class InputEmbeddings(nn.Module):
    ...
    def forward(self, x):
        # In the paper its mentioned the embeddings are multiplied with sqrt(d_model)
...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Question : what does torch.arange and unsqueeze do.
        # torch.arange(0, 5, dtype=torch.float)
        # Output: tensor([0., 1., 2., 3., 4.])

        # tensor = torch.arange(0, 5, dtype=torch.float)
        # tensor.unsqueeze(1)
        # Output:
        # tensor([[0.],
        #         [1.],
        ...
        # Multiplies each even index by 2i by the scaling factor to calculate \frac{2i}{d_{model}}
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))

        # torch.arange(0, 6, 2) - Creates a sequence of even indices from 0 to 6
        # Output: tensor([0, 2, 4])
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim = -1) # (Batch, h, seq_len, sew_len)
...
def build_transformer(src_vocab_size , tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff : int=2048)-> Transformer:
